...
{
	int nr = 0;
	ulong addr, pid_ptr, taskoffset;
	uint pid_level;

	if (VALID_MEMBER(task_struct_pids))
		taskoffset = OFFSET(task_struct_pids);
	else
		taskoffset = OFFSET(task_struct_pid_links);

	addr = task + taskoffset + (type * SIZE(pid_link)) + OFFSET(pid_link_pid);
				 ...
				 struct task_context *tc)
{
	...
	ENDIAN_ASSIGN(prstatus->pr_pgrp, 0);
	ENDIAN_ASSIGN(prstatus->pr_sid, 0);
				 ...
				 struct task_context *tc)
{
	...
	ENDIAN_ASSIGN(psinfo->pr_pgrp, 0);
	ENDIAN_ASSIGN(psinfo->pr_sid, 0);
